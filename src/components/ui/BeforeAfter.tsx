      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* BEFORE card (left) — fades OUT as slider goes right */}
            <div
              className="rounded-3xl p-8 border-2 transition-all duration-500"
              style={{
                opacity: 0.4 + ((100 - position) / 100) * 0.6,
                transform: `scale(${0.95 + ((100 - position) / 100) * 0.05})`,
              ...
              <div className="space-y-3">
                {beforeItems.map((item, i) => (
                  <div
                    ...
                    style={{ opacity: position <= (100 - i * 16) ? 1 : 0.2 }}
                  >
            ...
            {/* AFTER card (right) — fades IN as slider goes right */}
            <div
              className="rounded-3xl p-8 border-2 transition-all duration-500"
              style={{
                opacity: 0.4 + (position / 100) * 0.6,
                transform: `scale(${0.95 + (position / 100) * 0.05})`,
              ...
              <div className="space-y-3">
                {afterItems.map((item, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 text-sm transition-all duration-300"
                    style={{ opacity: position >= ((i + 1) * 16) ? 1 : 0.2 }}
                  >

import Link from "next/link";
import Image from "next/image";
import { Star, ShieldCheck, Cpu, FileCheck2 } from "lucide-react";
import type { BrandConfig } from "@/lib/constants";

function Stars({ rating, color }: { rating: number; color: string }) {
  const full = Math.floor(rating);
  const partial = rating - full;
  return (
    <span className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => {
        if (i < full) return <Star key={i} size={12} className={`fill-current ${color}`} />;
        if (i === full && partial >= 0.5) return (
          <span key={i} className="relative inline-block" style={{ width: 12, height: 12 }}>
            <Star size={12} className="absolute text-gray-300 fill-current" />
            <span className="absolute inset-0 overflow-hidden" style={{ width: "50%" }}>
              <Star size={12} className={`fill-current ${color}`} />
            </span>
          </span>
        );
        return <Star key={i} size={12} className="fill-current text-gray-300" />;
      })}
    </span>
  );
}

function Divider() {
  return <div className="hidden sm:block w-px h-4 bg-border shrink-0" />;
}

function BarShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-white border-b border-border/60">
      <div className="max-w-7xl mx-auto px-4 py-2.5">
        <div className="overflow-x-auto hide-scrollbar">
          <div className="flex items-center justify-start sm:justify-center gap-x-6 gap-y-2 min-w-max sm:min-w-0 sm:flex-wrap">
            {children}
          </div>
        </div>
      </div>
    </div>
  );
}

/**
 * Workwell trust strip. Deliberately conservative — only claims Workwell can
 * stand behind. Trustpilot rating comes from the brand registry; the rest are
 * service facts true of the offering. Add Workwell-specific accreditations
 * (FreeAgent partner, FCSA, etc.) here once confirmed.
 */
function WorkwellTrustBar({ brand }: { brand: BrandConfig }) {
  return (
    <BarShell>
      {brand.trustpilot && (
        <>
          <Link
            href={brand.trustpilot.url}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 shrink-0 whitespace-nowrap hover:opacity-80 transition-opacity"
          >
            <Stars rating={Number(brand.trustpilot.rating)} color="text-amber-400" />
            <span className="text-xs text-text-light">
              Rated <span className="text-dark font-semibold">Excellent on Trustpilot</span>
            </span>
          </Link>
          <Divider />
        </>
      )}

      <span className="flex items-center gap-1.5 shrink-0 whitespace-nowrap">
        <ShieldCheck size={14} className="text-primary" />
        <span className="text-xs font-semibold text-dark">Qualified &amp; Regulated</span>
      </span>

      <Divider />

      <span className="flex items-center gap-1.5 shrink-0 whitespace-nowrap">
        <FileCheck2 size={14} className="text-primary" />
        <span className="text-xs font-semibold text-dark">Making Tax Digital Ready</span>
      </span>

      <Divider />

      <span className="flex items-center gap-1.5 shrink-0 whitespace-nowrap">
        <Cpu size={14} className="text-primary" />
        <span className="text-xs font-semibold text-dark">Free Accounting Software</span>
      </span>
    </BarShell>
  );
}

/** Clever trust strip — unchanged content. */
function CleverTrustBar() {
  return (
    <BarShell>
      {/* Trustpilot */}
      <Link
        href="https://www.trustpilot.com/review/cleveraccounts.com"
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-1.5 shrink-0 whitespace-nowrap hover:opacity-80 transition-opacity"
      >
        <Stars rating={4.7} color="text-secondary" />
        <span className="text-xs text-text-light">Read our <span className="text-dark font-semibold">700+ Trustpilot Reviews</span></span>
      </Link>

      <Divider />

      {/* Google */}
      <div className="flex items-center gap-2 shrink-0 whitespace-nowrap">
        <svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        <span className="text-xs font-bold text-dark">Google</span>
        <Stars rating={4.4} color="text-[#FBBC05]" />
        <span className="text-xs text-text-light font-medium">4.4 · <span className="text-dark font-semibold">479 reviews</span></span>
      </div>

      <Divider />

      {/* FreeAgent Platinum Partner */}
      <a
        href="https://www.freeagent.com/find-an-accountant/clever-accounts-limited/"
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-2 hover:opacity-80 transition-opacity shrink-0 whitespace-nowrap"
        aria-label="FreeAgent Platinum Partner"
      >
        <Image
          src="/images/freeagentplat.jpg"
          alt=""
          width={105}
          height={28}
          className="h-7 w-auto object-contain"
        />
        <span className="text-xs font-semibold text-dark">FreeAgent</span>
      </a>

      <Divider />

      {/* FCSA Accredited */}
      <a
        href="https://www.fcsa.org.uk/"
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-2 hover:opacity-80 transition-opacity shrink-0 whitespace-nowrap"
        aria-label="FCSA Accredited"
      >
        <span className="relative inline-block h-7 w-7 overflow-hidden">
          <Image
            src="/images/FCSA.jpg"
            alt=""
            fill
            className="object-cover scale-[1.35]"
          />
        </span>
        <span className="text-xs font-semibold text-dark">FCSA</span>
      </a>

      <Divider />

      {/* Contracting Awards Finalist */}
      <div className="flex items-center gap-2 shrink-0 whitespace-nowrap">
        <Image
          src="/images/Award2020.jpg"
          alt=""
          width={28}
          height={28}
          className="h-7 w-auto object-contain"
        />
        <span className="text-xs font-semibold text-dark">Contracting Awards</span>
        <span className="text-xs text-text-light">2023, 2024, 2025</span>
      </div>
    </BarShell>
  );
}

export default function TrustBar({ brand }: { brand: BrandConfig }) {
  return brand.id === "workwell" ? <WorkwellTrustBar brand={brand} /> : <CleverTrustBar />;
}
